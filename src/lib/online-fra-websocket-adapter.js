...
function createOnlineFraWebSocketAdapter(options = {}) {
  ...
  function attestationFor(req) {
    ...
    if (!plain(value) || value.ok !== true || value.tlsSni !== config.hostname || value.clientVerify !== 'SUCCESS'
      || !SAFE_ID.test(value.deviceId) || !HEX.test(value.fingerprint) || !safeIp(value.ip)) return null;
    return Object.freeze({ deviceId: value.deviceId, fingerprint: value.fingerprint, ip: value.ip });
  }
  ...
  function admit(context, data, binary) {
    ...
    const connected = relayCall('connect', { identity, lease: parsed.lease });
  ...
  function onMessage(context, data, binary) {
    ...
    try {
      ...
      if (!binary || !Buffer.isBuffer(data) || data.length < 1 || data.length > config.maxFrameBytes
        || Number(context.ws.bufferedAmount || 0) > config.maxBufferedBytes) fail('ONLINE_FRA_WS_FRAME_INVALID');
      if (!refreshPeer(context)) fail('ONLINE_FRA_WS_PEER_UNAVAILABLE');
      relayCall('route', { connectionId: context.connectionId, peerConnectionId: context.peerConnectionId, frame: Buffer.from(data) });
      drain(context);
